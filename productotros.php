...
?>
 <div class="row ">
      ...
      <div class="col s1"><a class="btn-floating btn-large pulse disabled"><i class="material-icons">filter_7</i></a></div>
</div>
...
$query = "SELECT * from productos WHERE productos.codigo IN (SELECT codigo from unico) limit $itemPS,$totalE";
$result = $conexion->query($query);
while ($row = $result->fetch_assoc()){
	$img = $row['codigo'];
	$existen = $row['existen'];
...
echo "<center><h5>";
if ($pagA>=0){
	echo "<a href='productotros.php?pagi=$pagA'><img src='img/ant1.png' width='65' height='40'></a>";
	echo "&nbsp;";
	}
echo "<a href='productotros.php'><img src='img/home1.png' width='65' height='40'></a>";
echo "&nbsp;";
echo "<a href='productotros.php?pagi=$itemPS'><img src='img/sig1.png' width='65' height='40'></a>";
	...
	if (isset($COD) and isset($DESC)){
		...
		if (isset($COD)){
			//codigo de busqueda de codigo
					$query = "SELECT * from productos WHERE productos.codigo IN (SELECT codigo from unico WHERE codigo LIKE '%$COD%' ORDER BY codigo)";
					$result = $conexion->query($query);
					while ($row = $result->fetch_assoc()){
					$img = $row['codigo'];
					$existen = $row['existen'];
			...
			//codigo de busqueda de descripcion
					$query = "SELECT * from productos WHERE productos.descrip IN (SELECT descrip from unico WHERE descrip LIKE '%$DESC%' ORDER BY codigo)";
					$result = $conexion->query($query);
					while ($row = $result->fetch_assoc()){
					$img = $row['codigo'];
					$existen = $row['existen'];
